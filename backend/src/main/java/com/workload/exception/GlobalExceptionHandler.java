...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleDeviceNotInInventory(
        ...
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
                .body(
                        ApiResponse.error(
                                new ApiError(
                                        "DEVICE_NOT_IN_INVENTORY", ex.getMessage(), null)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleNoContextForSystem(
        ...
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
                .body(
                        ApiResponse.error(
                                new ApiError("NO_CONTEXT_FOR_SYSTEM", ex.getMessage(), null)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleRoundTripNotEditable(
        ...
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
                .body(
                        ApiResponse.error(
                                new ApiError("ROUND_TRIP_NOT_EDITABLE", ex.getMessage(), null)));
    }
